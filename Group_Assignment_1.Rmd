...
```{r}
portfolio <- HPR_daily %>%
  filter(stocks %in% c("msft","jpm","ge")) 
```
...
sp<- log(tot_ret$tot_sprtrn+1)

covariance<- data.frame(microsoft$LRET, jpm$LRET, ge$LRET, sp)
cov2<-data.frame((microsoft$LRET)^2, (jpm$LRET)^2, (ge$LRET)^2, (sp)^2)
cov(covariance)
cov(cov2)
```
...
```{r}
portfolio <- portfolio %>%
  mutate(ret_sq= RET^2,abs_ret=abs(RET))

acf(portfolio$RET)
acf(portfolio$ret_sq)
acf(portfolio$abs_ret)
```
```{r}
portfolio <-portfolio %>% 
  group_by(stocks) %>% 
...
#Selecting 0.4, 0.3, 0.3 as the weights
portfolio_ret <- 0.4*tot_ms$tot_RET + 0.3*tot_jpm$tot_RET +0.3*tot_ge$tot_RET
portfolio_ret

portfolio <- portfolio %>%
  mutate(port_ret=0.4*tot_RET_MS+0.3*tot_RET_JPM+ 0.3*tot_RET_GE)
```
...
portfolio %>% 
  group_by(date) %>%
  ggplot() + 
  geom_line(aes(x=date, y=tot_RET_MS), color='red') +
  geom_line(aes(x=date, y=tot_RET_JPM), color='yellow') +
  geom_line(aes(x=date, y=tot_RET_GE), color= 'green') +
  geom_line(aes(x=date, y=port_ret), color = 'black') +
  labs(title = '',
       subtitle = '',
       y = '') +
  NULL
